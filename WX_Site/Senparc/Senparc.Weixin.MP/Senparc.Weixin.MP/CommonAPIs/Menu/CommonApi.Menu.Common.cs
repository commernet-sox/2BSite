        ...
        private static void GetButtonGroup(List<MenuFull_RootButton> rootButtonList, ButtonGroupBase buttonGroup)
        {
            foreach (var rootButton in rootButtonList)
            {
                ...
                var availableSubButton = rootButton.sub_button == null
                    ? 0
                    : rootButton.sub_button.List.Count(z => z != null && !string.IsNullOrEmpty(z.name)); //可用二级菜单按钮数量
                if (availableSubButton == 0)
                {
                    ...
                    else if (rootButton.type.Equals("NEWS", StringComparison.OrdinalIgnoreCase))
                    {
                        //扫码推事件
                        buttonGroup.button.Add(new SingleViewButton()
                        {
                            name = rootButton.name,
                            url = rootButton.news_info.list[0].content_url,
                            type = "view"
                        });
                    }
                    else
                ...
                else
                {
                    ...
                    foreach (var subSubButton in rootButton.sub_button.List)
                    {
                        ...
                        else if (subSubButton.type.Equals("NEWS", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                name = subSubButton.name,
                                url = subSubButton.news_info.list[0].content_url,
                                type = "view"
                            });
                        }
                        else
