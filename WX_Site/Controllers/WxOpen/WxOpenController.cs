    ...
    public partial class WxOpenController : Controller
    {
        ...
        public ActionResult OnLogin(string code)
        {
            try
            {
                ...
                if (jsonResult.errcode == ReturnCode.请求成功)
                {
                    ...
                    //var preEmployeeService = _serviceProvider.GetService(typeof(IPreEmployeeService)) as IPreEmployeeService;

                    //var employee = employeeService.GetAll().Where(t => t.OpenID == jsonResult.openid).ToList();

                    //if (employee.Count < 1)
                    //{
                    //    var preEmployee = preEmployeeService.GetAll().Where(p => p.OpenID == jsonResult.openid && p.Status == 10).ToList();

                    //    if (preEmployee.Count < 1)
                    //    {
                    //        return Json(new { success = true, msg = "OK", sessionId = sessionBag.Key, sessionKey = sessionBag.SessionKey, sessionBag.ExpireTime, operation = "register", data = employeeIds });
                    //    }
                    //    else
                    //    {
                    ...
                    //    }
                    //}
                    //employeeIds = employee.Select(e => e.EmployeeId).ToList();
                    //var IsMaster = false;
                    //if (employeeIds.Count > 0)
                    //{
                    //    var workGroupService = _serviceProvider.GetService(typeof(IWorkGroupService)) as IWorkGroupService;
                    //    IsMaster = (from workgroup in workGroupService.GetAll()
                    //                where workgroup.Valid && workgroup.MasterId == employeeIds[0]
                    //                && workgroup.EffectiveDate.Date <= DateTime.Now.Date && workgroup.ExpirationDate >= DateTime.Now.Date
                    //                select workgroup
                    //      ).ToList().Count > 0 ? true : false;
                    //}

                    ////注意：生产环境下SessionKey属于敏感信息，不能进行传输！
                    //return Json(new { success = true, msg = "OK", sessionId = sessionBag.Key, sessionKey = sessionBag.SessionKey, sessionBag.ExpireTime, operation = "nothing", data = employeeIds, isMaster = IsMaster });
                }
                else
                {
        ...
        public ActionResult GetPrepayid(string sessionId, string content, int price = 1)
        {
            try
            {
                ...
                var body = string.IsNullOrEmpty(content) ? "E存储小程序微信支付" : content;
        ...
        public ActionResult CheckSession(string sessionId)
        {
            ...
            {
                sessionBag = SessionContainer.UpdateSession(sessionBag.OpenId, sessionBag.OpenId, sessionBag.SessionKey, sessionBag.UnionId);
                return Json(new { Success = true, Message = "", sessionBag.ExpireTime });
            }
        }
    }
}
