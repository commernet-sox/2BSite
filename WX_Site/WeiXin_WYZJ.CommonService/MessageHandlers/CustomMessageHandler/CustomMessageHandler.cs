...
namespace WX_CommonService.CustomMessageHandler
{
    /// <summary>
    /// 自定义MessageHandler
        ...
        public override IResponseMessageBase OnTextRequest(RequestMessageText requestMessage)
        {
            ...
            var requestHandler =
                requestMessage.StartHandler()
                  ...
                  //                    agentUrl = "http://www.ray-zhb.com/mp/weixin";
                  ...
                  .Keyword("命令", () =>
                 {
                     defaultResponseMessage.Content = @"回复命令关键字测试：
            ...
            //格式：【数字#数字】，如2010#0102，调用正则表达式匹配";
                ...
                .Keywords(new[] { "测试", "退出" }, () =>
                {
                    ...
                    if (Config.SenparcWeixinSetting.WeixinAppId == "wxaa7a2dce5b0333ba")//网赢之家
                    {
                        name = "网赢之家";
                    }
                    else if (Config.SenparcWeixinSetting.WeixinAppId == "wx61a271df6cbd2f8e")//如意仓
                    {
                        name = "如意仓";
                    }
                    else
                    ...
                    {
                        //进入APP测试
                        defaultResponseMessage.Content = "您已经进入【"+ name + "】的测试程序，请发送任意信息进行测试。发送文字【退出】退出测试对话。10分钟内无任何交互将自动退出应用对话状态。";
                    }
                    else
                    {
                        //退出APP测试
                        defaultResponseMessage.Content = "您已经退出【" + name + "】的测试程序。";
                    }
                    return defaultResponseMessage;
                })
                .Keyword("AsyncTest", () =>
                ...
                .Keyword("OPEN", () =>
                {
                    ...
                    授权之后，您的微信所收到的消息将转发到第三方（网赢之家）的服务器上，并获得对应的回复。

                    测试完成后，您可以登陆公众号后台取消授权。",
                            Url = "http://hrms.ruyicang.com/wyzj/home/index"
                        });
                        return openResponseMessage;
                    }
                    else
                    {
                        defaultResponseMessage.Content = string.Format("欢迎");
                        return defaultResponseMessage;
                    }
                ...
                //.Keyword("订阅", () =>
                //{
                //    defaultResponseMessage.Content = "点击打开：http://mp.ruyicang.com/ruyee/SubscribeMsg";
                //    return defaultResponseMessage;
                //})
                //正则表达式
                .Regex(@"^\d+#\d+$", () =>
