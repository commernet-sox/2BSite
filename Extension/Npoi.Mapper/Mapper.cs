...
namespace Npoi.Mapper
{
    ...
    public class Mapper
    {
        ...
        */
        ...
        public IEnumerable<RowInfo<T>> Take<T>(int sheetIndex = 0, int maxErrorRows = 10, Func<T> objectInitializer = null,int firstRowIndex=-1) where T : class
        {
            var sheet = Workbook.GetSheetAt(sheetIndex);
            return Take(sheet, maxErrorRows, objectInitializer, firstRowIndex);
        }
        ...
        private IEnumerable<RowInfo<T>> Take<T>(ISheet sheet, int maxErrorRows, Func<T> objectInitializer = null, int firstRowIndex = -1) where T : class
        {
            ...
            {
                targetType = GetDynamicType(sheet, firstRowIndex);
        ...
        private Type GetDynamicType(ISheet sheet, int firstRowIndex=-1)
        {
            if (firstRowIndex == -1)
                firstRowIndex = sheet.FirstRowNum;
            var firstRow = sheet.GetRow(firstRowIndex);
